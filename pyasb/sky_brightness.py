...
class SkyBrightness(object):
    ...
    def sky_brightness_region(BouguerFit, ImageInfo, fits_region_values):
        ...
        sky_brightness = BouguerFit.Regression.mean_zeropoint - \
            2.5 * \
            np.log10(sky_flux / (ImageInfo.exposure * ImageInfo.pixel_scale))
    ...
    def measure_in_grid(self, FitsImage, ImageInfo, ImageCoordinates, BouguerFit):
        ...
        def sky_brightness_point(az, zd):
            ...
            fits_region_values = FitsImage.fits_data[
                (np.array(ImageCoordinates.altitude_map >= alt - zdseparation / 2.) *
                 np.array(
                     ImageCoordinates.altitude_map < alt + zdseparation / 2.)
                 ) * (
                    (np.array(ImageCoordinates.azimuth_map >= az - azseparation / 2.) *
                     np.array(
                         ImageCoordinates.azimuth_map < az + azseparation / 2.)
                     ) +
                    (np.array(ImageCoordinates.azimuth_map >= az - azseparation / 2. + 360) +
                     np.array(
                         ImageCoordinates.azimuth_map < az + azseparation / 2. - 360)
                     ))]
            # The last two lines must be set to get a smooth transition between
            # 360 and 0 degrees azimuth
        ...
        self.SBgrid, self.SBgrid_errors = np.array(
            sky_brightness_list(self.AZgrid, self.ZDgrid))

        # Once we measured the sky brightness in the image, convert to radians
        # the azimuths
        ...
        try:
            ...
            for k, alt_ in enumerate(self.ALTdirs):
                ...
                for j in xrange(len(self.AZdirs)):
                    line += ', ' + \
                        str("%.3f" % float(
                            self.SBgrid[k][j])) + ' +/- ' + str("%.3f" % float(self.SBgrid_errors[k][j]))
                line += '\n'
                content.append(line)
        ...
        try:
            ...
            self.SBzenith, self.SBzenith_err = \
                self.sky_brightness_region(
                    BouguerFit, ImageInfo, fits_zenith_region_values)
...
class SkyBrightnessGraph(object):
    ...
    def plot_data(self):
        ...
        self.SBcontlabel = self.SBgraph.clabel(
            self.SBcontours, inline=True, fmt='%.1f', fontsize=10)
        # Limit radius
    ...
    def plot_labels(self, SkyBrightness, ImageInfo, BouguerFit):
        ...
        self.SBgraph.text(
            5 * np.pi / 4, 125, unicode(image_information, 'utf-8'), fontsize='x-small')
    ...
    def define_contours(self, ImageInfo):
        ...
        if len(self.level_list) < 3:
            self.update_ticks = False
        else:
            self.update_ticks = True
    ...
    def ticks_and_locators(self):
        ...
        theta_label = [
            "$N$", "$NE$", "$E$", "$SE$", "$S$", "$SW$", "$W$", "$NW$"]
        self.SBgraph.set_rgrids(
            radial_locator, radial_label, size="large", color='k', alpha=0.75)
    ...
    def color_bar(self):
        ...
        # Color bar
        self.SBcolorbar = plt.colorbar(
            self.SBcontoursf, orientation='vertical', shrink=0.85)
        self.SBfigure.subplots_adjust(right=0.80)  # Restore separation
        self.SBcolorbar.set_ticks(
            self.label_list, update_ticks=self.update_ticks)
        self.SBcolorbar.set_label(
            "mag/arcsec2", rotation="vertical", size="large")
