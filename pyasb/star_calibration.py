...
class Star(object):
    ...
    def __init__(self, StarCatalogLine, FitsImage, ImageInfo, ObsPyephem):
        ...
        if self.destroy == False:
            ...
            if self.destroy == True:
                if DEBUG == True:
                    print(
                        self.name + ' Error reading used filter or magnitude>max')
        ...
        if self.destroy == False:
            ...
            if self.destroy == True:
                if DEBUG == True:
                    print(
                        self.name + ' Error performing star astrometry, maybe star isnt visible?')
        ...
        if self.destroy == False:
            ...
            if self.destroy == True:
                if DEBUG == True:
                    print(
                        self.name + ' Error creating regions of stars and star+background')
    ...
    def estimate_centroid(self, iterations=1):
        ...
        try:
            ...
            xweight = np.array(
                [range(1, len(self.fits_region_star[0]) + 1)] * len(self.fits_region_star))
            yweight = np.array(
                [range(1, len(self.fits_region_star) + 1)] * len(self.fits_region_star[0])).transpose()
...
class StarCatalog(object):
    ...
    def save_to_file(self, ImageInfo):
        try:
            ...
            content = [
                '#HDcode, CommonName, RA1950, DEC1950, Azimuth, Altitude, Airmass, Magnitude, Color(#-V), StarFlux, StarFluxErr, mag+2.5logF, [mag+2.5logF]_Err\n']
